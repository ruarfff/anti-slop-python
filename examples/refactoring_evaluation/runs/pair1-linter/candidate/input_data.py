...
def parse_order(row: dict[str, str], lines: Sequence[OrderLine]) -> Order:
    ...
    return Order(
        required_text(row, "order_id"),
        required_text(row, "customer_id"),
        date.fromisoformat(required_text(row, "placed_on")),
        shipping,
        discount,
        tuple(lines),
    )
...
def load_customers(directory: Path) -> dict[str, Customer]:
    ...
    for row in read_rows(directory / "customers.csv"):
        ...
        if item.identifier in result:
            raise ValueError(f"Duplicate customer: {item.identifier}")
...
def load_products(directory: Path) -> dict[str, Product]:
    ...
    for row in read_rows(directory / "products.csv"):
        item = parse_product(row)
        if item.sku in result:
            raise ValueError(f"Duplicate product: {item.sku}")
...
def load_orders(directory: Path) -> list[Order]:
    ...
    for row in read_rows(directory / "orders.csv"):
        ...
        result.append(parse_order(row, groups.pop(order_id, [])))
